...
parser.add_argument('--threshold', type=int, default=210, help='plot全体の色の濃さの閾値')
parser.add_argument('-m','--method', choices=['bgr2gray','hsv'], default='bgr2gray', help='plot全体の色の濃さを定量する方法')
parser.add_argument('--crop', frag=False, help='画像の余白が白い場合に、余白を削除するかどうか')
...
class img_di_ratio(object):
	...
	def __init__(self, img, threshold, method, crop):
		self.img = img
	...
	def convert_gray(self):
		if self.method is 'bgr2gray':
			# convert to gray scale
			gray = cv2.cvtColor(self.img, cv2.COLOR_BGR2GRAY)

		elif self.method is 'hsv':
...
if __name__ == '__main__':
	...
	# get the plot which is only plotting data ink
	img = get_img_from_py('data_ink.py', 'data_ink.png')
	data_ink = cv2.imread('data_ink.png')
